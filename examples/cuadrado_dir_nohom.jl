...
dD(x)  = (x[1:1,:] .- x[1:1,:].^2).*(x[2:2,:] .- x[2:2,:].^2)

dist = Distance(dD,nothing)
...
f(x) = exp.(x[1:1,:]) .* ( Π*cos.(Π*x[2:2,:]) .- sin.(Π*x[2:2,:]) )

#Boundary Data
gD(x) = (exp(1.0f0)-1)*x[1:1,:].*sin.(Π*x[2:2,:])
bd = BoundaryData(gD,nothing,nothing)
# SOLUCIÓN EXACTA
U(x) = (exp.(x[1:1,:]) .- 1.0f0).*sin.(Π*x[2:2,:])
